...
import com.etna.mycalendar.Activity.DisplayUserGroupeAgendaActivity
...
class DisplayGroupeAdapter(mContext: Context, mUserModels: MutableList<UserModel?>?) :
    RecyclerView.Adapter<DisplayGroupeAdapter.ViewHolder>() {
    private val mContext: Context = mContext
    private val mUserModels: MutableList<UserModel?>? = mUserModels

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view: View =
            LayoutInflater.from(mContext).inflate(R.layout.user_of_groupe_item, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        inflater.inflate(R.menu.display_groupe_profil_menu, popup.menu)
        ...
        popup.setOnMenuItemClickListener(PopupMenu.OnMenuItemClickListener { item ->
            when (item.itemId) {
                R.id.seeAgenda -> {
                    val intentGroupeAgenda = Intent(
                        mContext,
                        DisplayUserGroupeAgendaActivity::class.java
                    )
                    intentGroupeAgenda.putExtra("idUser", userModel?.id)
                    mContext.startActivity(intentGroupeAgenda)
